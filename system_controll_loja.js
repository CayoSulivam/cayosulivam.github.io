...
function showLoadingState() {
  ...
  window.showCRUDModal = async (action, collection, itemId = null) => {
    ...
    modalSaveBtn.onclick = async () => {
      const formData = {}; // Declaração da variável formData
      const fileInputs = {};
      ...
      try {
        ...
        for (const [fieldName, input] of Object.entries(fileInputs)) {
          if (input.files.length > 0) {
            const file = input.files[0];
            const filePath = `${collection}/${Date.now()}_${file.name}`;
            const fileRef = storage.ref().child(filePath);
            await fileRef.put(file);
            formData[fieldName] = await fileRef.getDownloadURL();
          } else if (action === 'edit' && itemData[fieldName]) {
            // Manter arquivo existente se não for substituído
            formData[fieldName] = itemData[fieldName];
          }
        }
  
        // Adicionar timestamp para novos registros
        if (action === 'add') {
